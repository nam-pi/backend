...
@Slf4j
public abstract class AbstractHydraBuilderOld<T extends AbstractHydraBuilderOld<T>>
    implements InterfaceHydraBuilderOld {
  ...
  public AbstractHydraBuilderOld(Property mainType) {
    this.mainType = mainType;
    this.mainWhere.addPrefix("xsd", XSD.getURI()).addPrefix("rdfs", RDFS.getURI()).addPrefix("core", Core.getURI())
        .addWhere(MAIN_SUBJ, RDF.type, this.mainType).addWhere(MAIN_SUBJ, RDFS.label, MAIN_LABEL);
    this.builder.addPrefix("doc", Doc.getURI()).addPrefix("core", Core.getURI()).addPrefix("hydra", Hydra.getURI())
