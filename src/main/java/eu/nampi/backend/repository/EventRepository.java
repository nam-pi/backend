...
public class EventRepository extends AbstractHydraRepository {
  ...
  private static final Node VAR_DATE_REAL_SORT_OUTER = NodeFactory.createVariable("dateRealSortOuter");
  ...
  private static final Node VAR_MAIN_PARTICIPANT_LABEL = NodeFactory.createVariable("mainParticipantLabel");
  ...
  private static final Node VAR_PARTICIPATION_TYPE = NodeFactory.createVariable("participationType");
  ...
  private static final BiFunction<Model, QuerySolution, RDFNode> ROW_MAPPER = (model, row) -> {
    ...
    model.add(main, Core.isInterpretationOf, act).add(act, RDF.type, Core.act).add(act, Core.isAuthoredBy, author)
        .add(author, RDF.type, Core.author).add(author, RDFS.label, row.getLiteral(VAR_AUTHOR_LABEL.toString()))
        .add(act, Core.isAuthoredOn, date).add(date, RDF.type, Core.date)
        .add(date, Core.hasDateTime, row.getLiteral(VAR_ACT_DATE_TIME.toString()))
        .add(act, Core.hasSourceLocation, location).add(location, Core.hasText, locationText)
        .add(location, Core.hasSource, source).add(source, RDF.type, Core.source)
    ...
    Optional.ofNullable(row.getResource(VAR_LOCATION_TYPE.toString())).ifPresentOrElse(
        type -> model.add(location, RDF.type, type), () -> model.add(location, RDF.type, Core.sourceLocation));
    ...
    // Aspect
    Optional.ofNullable(row.getResource(VAR_ASPECT.toString())).ifPresent(aspect -> {
      ...
      Optional.ofNullable(row.getResource(VAR_ASPECT_TYPE.toString()))
          .ifPresentOrElse(type -> model.add(aspect, RDF.type, type), () -> model.add(aspect, RDF.type, Core.aspect));
    });
    // Participant
    Optional.ofNullable(row.getResource(VAR_PARTICIPANT.toString())).ifPresent(agent -> {
      model.add(main, Core.hasParticipant, agent).add(agent, RDFS.label,
          row.getLiteral(VAR_PARTICIPANT_LABEL.toString()));
      Optional.ofNullable(row.getResource(VAR_PARTICIPATION_TYPE.toString()))
          .map(type -> ResourceFactory.createProperty(type.getURI()))
          .ifPresentOrElse(type -> model.add(main, type, agent), () -> model.add(main, Core.hasParticipant, agent));
      Optional.ofNullable(row.getResource(VAR_PARTICIPANT_TYPE.toString()))
          .ifPresentOrElse(type -> model.add(agent, RDF.type, type), () -> model.add(agent, RDF.type, Core.agent));
    });
    // Main participant
    Optional.ofNullable(row.getResource(VAR_MAIN_PARTICIPANT.toString()))
        .ifPresent(participant -> model.add(main, Core.hasMainParticipant, participant).add(participant, RDFS.label,
            row.getLiteral(VAR_MAIN_PARTICIPANT_LABEL.toString())));
    // Place
    Optional.ofNullable(row.getResource(VAR_PLACE.toString())).ifPresent(place -> {
      model.add(main, Core.takesPlaceAt, place).add(place, RDFS.label, row.getLiteral(VAR_PLACE_LABEL.toString()));
      Optional.ofNullable(row.getResource(VAR_PLACE_TYPE.toString()))
          .ifPresentOrElse(type -> model.add(place, RDF.type, type), () -> model.add(place, RDF.type, Core.place));
    });
    ...
    Optional.ofNullable(row.getLiteral(VAR_DATE_TIME_EXACT.toString())).ifPresent(dateTime -> {
      ...
      model.add(main, Core.takesPlaceOn, resDate).add(resDate, RDF.type, Core.date).add(resDate, Core.hasDateTime,
          dateTime);
    });
    // Earliest date
    Optional.ofNullable(row.getLiteral(VAR_DATE_TIME_EARLIEST.toString())).ifPresent(dateTime -> {
      ...
      model.add(main, Core.takesPlaceNotEarlierThan, resDate).add(resDate, RDF.type, Core.date).add(resDate,
          Core.hasDateTime, dateTime);
    });
    // Latest date
    Optional.ofNullable(row.getLiteral(VAR_DATE_TIME_LATEST.toString())).ifPresent(dateTime -> {
      ...
      model.add(main, Core.takesPlaceNotLaterThan, resDate).add(resDate, RDF.type, Core.date).add(resDate,
          Core.hasDateTime, dateTime);
    });
    // Sorting date
    Optional.ofNullable(row.getLiteral(VAR_DATE_OUTER.toString())).ifPresent(dateTime -> {
      ...
      model.add(main, Core.hasSortingDate, resDate).add(resDate, RDF.type, Core.date).add(resDate, Core.hasDateTime,
          dateTime);
    });
    return main;
  };
  ...
  public String findAll(QueryParameters params, Lang lang, Optional<String> dates, Optional<Resource> aspect,
    ...
    dates.ifPresent(datesString -> {
      ...
      Optional<Expr> start = dateRange.getStart().map(date -> ef.asExpr(ResourceFactory
          .createTypedLiteral(date.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), XSDDateType.XSDdateTime)));
      Optional<Expr> end = dateRange.getEnd().map(date -> ef.asExpr(ResourceFactory
          .createTypedLiteral(date.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME), XSDDateType.XSDdateTime)));
      if (start.isPresent() || end.isPresent()) {
        if (start.isPresent() && end.isPresent()) {
          builder.coreData.addFilter(ef.ge(VAR_DATE, start.get())).addFilter(ef.le(VAR_DATE, end.get()));
    ...
    builder.extendedData.addWhere(datesWhere(order, VAR_DATE_REAL_SORT_OUTER, VAR_DATE_OUTER)).addWhere(actWhere(false))
        .addWhere(participantWhere(false)).addOptional(aspectWhere(false)).addOptional(placeWhere(false));
  ...
  private WhereBuilder actWhere(boolean withTypes) {
    ...
    builder.addWhere(VAR_MAIN, Core.isInterpretationOf, VAR_ACT).addWhere(VAR_ACT, Core.isAuthoredBy, VAR_AUTHOR)
        .addWhere(VAR_AUTHOR, RDFS.label, VAR_AUTHOR_LABEL).addWhere(VAR_ACT, Core.isAuthoredOn, VAR_ACT_DATE)
        .addWhere(VAR_ACT_DATE, Core.hasDateTime, VAR_ACT_DATE_TIME)
        .addWhere(VAR_ACT, Core.hasSourceLocation, VAR_LOCATION).addWhere(VAR_LOCATION, Core.hasText, VAR_LOCATION_TEXT)
        .addWhere(VAR_LOCATION, Core.hasSource, VAR_SOURCE).addWhere(VAR_SOURCE, RDFS.label, VAR_SOURCE_LABEL);
    if (withTypes) {
      builder.addWhere(VAR_LOCATION, RDF.type, VAR_LOCATION_TYPE)
  ...
  private WhereBuilder aspectWhere(boolean withTypes) {
    WhereBuilder builder = new WhereBuilder();
    ExprFactory ef = builder.getExprFactory();
    builder.addWhere(VAR_MAIN, Core.usesAspect, VAR_ASPECT).addWhere(VAR_ASPECT, RDFS.label, VAR_ASPECT_LABEL)
  ...
  private WhereBuilder datesWhere(Order order, Node varDate, Node varDateTime) {
    ...
    ExprFactory ef = builder.getExprFactory();
    builder
        .addOptional(new WhereBuilder().addWhere(VAR_MAIN, Core.takesPlaceOn, VAR_DATE_EXACT).addWhere(VAR_DATE_EXACT,
            Core.hasDateTime, VAR_DATE_TIME_EXACT))
        .addOptional(new WhereBuilder().addWhere(VAR_MAIN, Core.takesPlaceNotEarlierThan, VAR_DATE_EARLIEST)
            .addWhere(VAR_DATE_EARLIEST, Core.hasDateTime, VAR_DATE_TIME_EARLIEST))
        .addOptional(new WhereBuilder().addWhere(VAR_MAIN, Core.takesPlaceNotLaterThan, VAR_DATE_LATEST)
            .addWhere(VAR_DATE_LATEST, Core.hasDateTime, VAR_DATE_TIME_LATEST))
        .addOptional(new WhereBuilder().addWhere(VAR_MAIN, Core.hasSortingDate, VAR_DATE_SORT).addWhere(VAR_DATE_SORT,
            Core.hasDateTime, VAR_DATE_TIME_SORT))
        .addBind(
                ...
                ef.cond(ef.bound(VAR_DATE_EXACT), ef.asVar(VAR_DATE_EXACT),
                    order.equals(Order.ASCENDING)
                        ? ef.asExpr(ef.cond(ef.bound(VAR_DATE_LATEST), ef.asVar(VAR_DATE_LATEST),
                            ef.cond(ef.bound(VAR_DATE_EARLIEST), ef.asVar(VAR_DATE_EARLIEST), ef.bnode())))
                        : ef.asExpr(ef.cond(ef.bound(VAR_DATE_EARLIEST), ef.asVar(VAR_DATE_EARLIEST),
                            ef.cond(ef.bound(VAR_DATE_LATEST), ef.asVar(VAR_DATE_LATEST), ef.bnode()))))),
            varDate)
        .addBind(
            ef.cond(
                        ...
                        order.equals(Order.ASCENDING)
                            ? ef.asExpr(ef.cond(ef.bound(VAR_DATE_TIME_LATEST), ef.asVar(VAR_DATE_TIME_LATEST),
                                ef.cond(ef.bound(VAR_DATE_TIME_EARLIEST), ef.asVar(VAR_DATE_TIME_EARLIEST),
                                    ef.asExpr(ResourceFactory.createTypedLiteral(POSITIVE_DEFAULT_DATE,
  ...
  private WhereBuilder participantWhere(boolean withTypes) {
    WhereBuilder builder = new WhereBuilder();
    ExprFactory ef = builder.getExprFactory();
    builder.addWhere(VAR_MAIN, Core.hasParticipant, VAR_PARTICIPANT)
    ...
    if (withTypes) {
      builder.addWhere(VAR_MAIN, VAR_PARTICIPATION_TYPE, VAR_PARTICIPANT)
  ...
  private WhereBuilder placeWhere(boolean withTypes) {
    WhereBuilder builder = new WhereBuilder();
    ExprFactory ef = builder.getExprFactory();
    builder.addWhere(VAR_MAIN, Core.takesPlaceAt, VAR_PLACE).addWhere(VAR_PLACE, RDFS.label, VAR_PLACE_LABEL);
    if (withTypes) {
