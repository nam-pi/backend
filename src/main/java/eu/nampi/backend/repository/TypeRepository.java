...
public class TypeRepository extends AbstractHydraRepository {
  ...
  @Cacheable(
      key = "{#lang, #params.limit, #params.offset, #params.orderByClauses, #params.type}")
  public String findAll(QueryParameters params, Lang lang) {
    // Try to get results as class
    HydraCollectionBuilder classesBuilder =
        new HydraCollectionBuilder(jenaService, endpointUri("types"),
    ...
    while (iterator.hasNext()) {
      int totalItems =
          (Integer) iterator.next().asTriple().getObject().getLiteral().getValue();
      if (totalItems > 0) {
        return serialize(classesBuilder.model, lang, classesBuilder.root);
      }
    }
    // Try to get results as property
    HydraCollectionBuilder propertiesBuilder =
        new HydraCollectionBuilder(jenaService, endpointUri("types"),
            RDFS.Resource, Api.typeOrderByVar, params, false, false);
    propertiesBuilder.coreData.addWhere(VAR_MAIN, RDFS.subPropertyOf,
        params.getType().map(ResourceFactory::createResource).orElseThrow());
    propertiesBuilder.build(ROW_MAPPER);
    return serialize(propertiesBuilder.model, lang, propertiesBuilder.root);
  }
